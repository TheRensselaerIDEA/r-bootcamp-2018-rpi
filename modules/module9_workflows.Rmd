---
title: "R Bootcamp, Module 9: </br>Workflows, Managing Projects and Good Practices"
...
# First things first: RStudio Cheat Sheets!

+ Via RStudio Help Menu
![](img/cheatsheets.png)
+ Via the web [https://www.rstudio.com/resources/cheatsheets/](https://www.rstudio.com/resources/cheatsheets/)

# Tips for avoiding bugs (1)
...
- Code in a modular fashion
    - Make use of functions, so that you don't need to debug the same code multiple times. 
...
- Use "core R"" functionality and algorithms already coded. 
    - Figure out if a functionality already exists in (or can be adapted from) an R
package (or potentially in a C/Fortran library/package): 
    - Code that is part of standard mathematical/numerical packages will probably
be more efficient and bug-free than anything you would write.

# Tips for avoiding bugs (2)

- Remove objects you don't need, to avoid accidentally using values from an old object via the scoping rules.
...
- Don't hard code numbers - use variables (e.g., number of iterations, parameter values in simulations), even if you don't expect to change the value
    - as this makes the code more readable and reduces bugs when
you use the same number multiple times
- Check that inputs to and outputs from functions (either functions you call or functions you write) are valid
    - use `warning()` and `stop()` to give a warning or stop execution when something unexpected happens.
- Use `try()` with functions that may fail in cases where you don't want overall execution to fail because a single piece of the execution fails.
- Write code for clarity and accuracy first; then worry about efficiency.
    - Write an initial version of the code in the simplest way, without trying to be efficient (e.g., you might use \emph{for} loops even if you're coding in R); 
    - Make a second version that employs efficiency tricks and check that both produce the same output.
...
[http://bit.ly/rbootcamp2018survey](http://bit.ly/rbootcamp2018survey)
