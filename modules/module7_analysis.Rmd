---
title: "R Bootcamp, Module 7: </br>Analysis"
...
+ Once we've carried out group-wise operations and perhaps reshaped it, we may also like to describe the relationships in the data. 
+ Often this involves fitting some style of *regression model.*  
+ The goal can be pure prediction, description, or inferring a causal relationship between variables.

Of course to infer causality, one has to be quite careful; techniques that try to avoid the usual pitfall that **correlation is not causation** are way beyond what we can cover here.

We'll just see the basics of how to fit regressions here; for more, take e.g. "Intro to Data Math" or "Intro to Biostatistic" or...
...
+ There are several families and links to use (`help(family)` for a full list), but some essentials:
...
- When we store this regression in an *object*, we get access to several items of interest. Let's take a look:
...
reg$aic
# See the standard (diagnostic) plots for a regression object
# Do ?plot.lm for more info!
plot(reg)
```
...
- Note that, in our results, R has broken up our variables into their different factor levels
    + It will do this whenever your regressors have factor levels
...
# A bit more about model-building 
...
+ It's a good idea to use `seq_along()` and `seq_len()` and not syntax like `1:length(air)` in `sample()` because the outcome of `length()` might in some cases be unexpected (e.g., if you're taking subsets of a dataset). 
