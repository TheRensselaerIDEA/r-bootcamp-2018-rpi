---
title: "R Bootcamp, Module 5: </br>Data Manipulation Using the tidyverse"
date: "Updated 17 Feb 2021 (JSE)"
author:
...
* "We should stick to the base R functions for manipulating and tidying"
    * `tidyverse` uses syntax that's unlike base R and is superfluous.
    * This view is increasingly fading
* "We should start teaching students to manipulate data using `tidyverse` tools"
    * `tidyverse` methods are easy to use, more readable than base R, and speed up the tidying process.
    * This is the dominant view, and is what we believe!
...
knitr::include_graphics('./img/tidyverse_cheatsheet.png')
```
...
We want our code to more closely resemble our _thinking_, which is "get the means by `continent`"
...
* The second step is the `select()` function. In this case we don't specify the data object to use in the call to `select()` since we've piped it in. 
...
In other words, when using the `group_by()` function, we split our original dataframe into multiple pieces, which we then apply summary functions to (e.g. `mean()` or `sd()`) within `summarize()`.
...
That allowed us to calculate the mean `gdpPercap` for each continent. But it gets even better -- the function `group_by()` allows us to group by _multiple_ variables. Let's group by `year` and `continent`.
...
* We have three "ID variables" (`continent`, `country`, `year`)
* We have three "Observation variables" (`pop`, `lifeExp`, `gdpPercap`)
...
1. the name for the _new_ column for the new ID variable (`obstype_year`), 
2. the name for the _new_ amalgamated observation variable (`obs_value`), 
