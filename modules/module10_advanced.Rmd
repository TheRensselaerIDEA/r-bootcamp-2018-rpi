---
title: "R Bootcamp, Module 10: </br>Advanced Topics"
...
For some of the topics here, our goal is not to teach you how to fish, but merely to tell you that fish exist, they are delicious, that they can be caught, and where one might go to figure out how to catch them.
...
```{r chunk-2}
rboot2018 <- list(month = 'January', year = 2018, 
  instructor = 'Erickson', attendance = 80)
...
You can get an example database of information about Stack Overflow questions and answers from [http://www.stat.berkeley.edu/share/paciorek/stackoverflow-2016.db](http://www.stat.berkeley.edu/share/paciorek/stackoverflow-2016.db) -- 619MB! (Stack Overflow is a website where programmers and software users can pose questions and get answers to technical problems)
...
* Mac/Linux - use *top* or *htop* or *ps*
...
* For Linux, *openBLAS*, Intel's *MKL* and AMD's *ACML* are fast and threaded. RPI's IDEA Cluster uses *MKL*!
...
We'll demonstrate that my Linux laptop is using multiple cores for linear algebra operations.
...
```{r chunk-14, cache=TRUE, eval=FALSE}
# Note: Might not work in RStudio
require(parallel)
require(doParallel)
require(foreach)
require(mgcv)
...
```{r chunk-15, cache=TRUE, eval=FALSE}
require(parallel)
nCores <- 2
...
```{r chunk-16, cache=TRUE, eval=FALSE}
require(parallel)
nCores <- 2
...
If you have access to multiple machines within a networked environment, such as the Rensselaer IDEA Cluster, there are a few (sometimes) straightforward ways to parallelize EP jobs across machines.
...
See Chris Paciorek's [tutorial on distributed computing](https://github.com/berkeley-scf/tutorial-parallel-distributed) for syntax and more details (UC Berkeley examples)
